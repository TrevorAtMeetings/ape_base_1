...
def get_chart_data(pump_code):
    ...
    try:
        ...
        # Get the best curve and calculate performance using Brain system
        # Brain system now handles this through evaluate_pump method
        evaluation_result = brain.evaluate_pump(pump_code, flow_rate, head)
        best_curve = curves[0] if curves else None  # Use first available curve
        solution = evaluation_result if evaluation_result else None
        ...
        if solution:
            ...
            if sizing_info and sizing_info.get(
                ...
                if not base_speed and curves and len(curves) > 0:
                    base_speed = curves[0].get('test_speed_rpm"), 2900)
...
def get_chart_data_safe(safe_pump_code):
    ...
    try:
        ...
        # Calculate BEP analysis using Brain system
        bep_analysis = None  # Brain system handles this internally
        ...
        for i, curve in enumerate(curves):
            ...
            curve_data = {
                'curve_index': i,
                'curve_id': f"{target_pump.get('pump_code', pump_code)}_C{i+1}_{final_diameter:.1f}mm",
