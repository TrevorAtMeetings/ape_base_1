...
class ConfigManager:
    ...
    def _load_config(self):
        ...
        try:
            ...
            with open(config_path, 'r') as f:
                ...
                if not self._config:
                    raise ValueError(
                        "Configuration file is missing 'hardcoded_values' section"
                    )
            ...
            print(
                "Please ensure app/brain/brain_config/config.json exists with proper configuration"
            )
            self._set_defaults()  # Use emergency defaults
            print(
                "WARNING: Using emergency default values - system may not function correctly"
            )
            ...
            print(
                "Please fix the JSON syntax in app/brain/brain_config/config.json"
            )
            self._set_defaults()  # Use emergency defaults
            print(
                "WARNING: Using emergency default values - system may not function correctly"
    ...
    def _process_config(self):
        ...
        self.performance_affinity = self._extract_values(
            'performance_affinity_constants')
        self.performance_curves = self._extract_values(
        ...
        self.hydraulic_classifier = self._extract_values(
            'hydraulic_classifier_constants')
        self.proximity_searcher = self._extract_values(
            'proximity_searcher_constants')
        self.pump_evaluator = self._extract_values('pump_evaluator_constants')
        self.performance_vfd = self._extract_values(
    ...
    def _validate_required_keys(self):
        ...
        required_sections = [
            'pump_evaluator', 'performance_vfd', 'validation',
            'performance_affinity', 'physics_models', 'dynamic_physics'
        ]
        ...
        for section_name in required_sections:
            ...
            if len(value_keys) == 0:
                self._validation_errors.append(
                    f"Section '{section_name}' has no configuration values")
    ...
    def _extract_values(self, section_name: str) -> Dict[str, Any]:
        ...
        for item in section:
            ...
            if 'constant' in item:
                ...
                key = description.lower().replace(' ', '_').replace(
                    '(', '').replace(')', '').replace('/', '_')
                # Remove special characters
                key = ''.join(c if c.isalnum() or c == '_' else ''
                              for c in key)
    ...
    def _set_defaults(self):
        ...
        # Cache defaults
        self.cache = {'max_size_value': 1000, 'default_ttl_value': 300}
