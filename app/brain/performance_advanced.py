...
============================
Advanced performance calculations including industry standard methods, 
efficiency optimization, and VFD calculations
"""
...
class PerformanceAdvancedCalculator:
    """Advanced performance calculations and industry standard methods"""
        ...
        try:
            ...
            try:
                ...
                if 'diffuser' in pump_type or 'turbine' in pump_type:
                    ...
                    efficiency_penalty_factor = self._get_calibration_factor('efficiency_penalty_diffuser', 0.45)
                    pump_type_classification = "diffuser"
                else:
                    # Volute pumps (default): Lower efficiency penalty (research: 0.15-0.25)
                    efficiency_penalty_factor = self._get_calibration_factor('efficiency_penalty_volute', 0.20)
                ...
                try:
                    ...
                    if npsh_values and len(npsh_values) == len(flows_sorted):
                        ...
                        if not np.isnan(base_npshr):
                            ...
                            npsh_threshold = self._get_calibration_factor('npsh_degradation_threshold', 10.0)
                            if trim_percent is not None and trim_percent < (100 - npsh_threshold):  # More than 10% trim
                                npsh_degradation_factor = self._get_calibration_factor('npsh_degradation_factor', 1.15)
                ...
                if original_bep_flow > 0 and original_bep_head > 0 and diameter_ratio < 1.0:
                    ...
                    if true_qbp_percent > 110:  # Operating significantly above shifted BEP
                        ...
                        efficiency_correction_factor = self._get_calibration_factor('efficiency_correction_exponent', 0.1)
    ...
    def _get_calibration_factor(self, factor_name: str, default_value: float = 1.0) -> float:
        ...
        try:
            ...
            for trim_percent in test_trims:
                ...
                if original_bep_flow > 0 and original_bep_head > 0 and diameter_ratio < 1.0:
                    ...
                    flow_exponent = physics_exponents['flow_exponent_x'] if physics_exponents else self._get_calibration_factor('bep_shift_flow_exponent', 1.2)
                    head_exponent = physics_exponents['head_exponent_y'] if physics_exponents else self._get_calibration_factor('bep_shift_head_exponent', 2.2)
