...
# Note: _calculate_extrapolated_estimate method removed - we no longer provide estimates for impossible trim scenarios
    ...
    def calculate_performance_with_speed_variation(self, pump_data: Dict[str, Any], 
        ...
        This method finds the required speed to operate a pump at the target conditions
        by finding a corresponding point on the reference curve that lies on the same
        system curve as the target point.
        
        Args:
            pump_data: Complete pump data dictionary with curves and specifications
            target_flow: Target flow rate (Q₂) in m³/hr
            target_head: Target head (H₂) in meters
            h_static_ratio: Ratio of static head to total head (default 0.4 = 40%)
        
        Returns:
            Performance dictionary with VFD sizing information, or None if not feasible
            
        The return dictionary contains:
            - meets_requirements: bool
            - flow_m3hr: target flow
            - head_m: target head  
            - efficiency_pct: efficiency at operating point
            - power_kw: power consumption
            - sizing_method: 'Speed Variation'
            - required_speed_rpm: calculated speed
            - reference_speed_rpm: test/nominal speed
            - speed_ratio: n₂/n₁ as percentage
            - reference_point: dict with Q₁, H₁ on original curve
            - system_curve_k: calculated system curve constant
        """
        try:
            pump_code = pump_data.get('pump_code', 'Unknown')
            ...
            # Flow at new speed: Q₂ = Q₁ * (n₂/n₁) = Q₁ * speed_ratio
            # But we want target_flow, so we need to verify this matches
